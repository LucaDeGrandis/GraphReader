...
Notebook Content (delimited by ```): ```<NOTEBOOK>```
...
ATOMIC_FACTS_TEMPLATE = """- {chunk_id}: {fact}\n"""
...
def read_node_atomic_facts_generation(
    ...
    **kwargs,
    ...
    atomic_facts_str = ""
    for i, fact_list in enumerate(kwargs['atomic_facts']):
        for fact in fact_list:
            atomic_facts_str += ATOMIC_FACTS_TEMPLATE.format(**{
                'chunk_id': i,
                'fact': fact,
            })
    atomic_facts_str = atomic_facts_str.strip()
